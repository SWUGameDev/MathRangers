...
namespace TexDrawLib
{
    ...
    public static class TexParserUtility
    {
        ...
        static private bool IsItVerbatimThenSkip(string text, ref int position)
        {
            const string verbatimToken = "\\verb";
            if (text.StartsWithOffset(verbatimToken, position))
            {
                position += verbatimToken.Length;
                if (position == text.Length)
                    return true;
                char gate = text[position++];
                var end = text.IndexOf(gate, position);
                if (end >= 0)
                    position = end + 1;
                else
        ...
        static public string ReadGroup(string text, ref int position, char openChar = beginGroupChar, char closeChar = endGroupChar)
        {
            ...
            while (position < text.Length && !(text[position] == closeChar && group == 0))
            {
                ...
                {
                    if (IsItVerbatimThenSkip(text, ref position))
                    {
                        continue;
                    }
                    position++;
